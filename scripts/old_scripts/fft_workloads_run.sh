# cd ../fft_workload
# g++ -O0 \-nostdlib -statcd gemic -ffreestanding -fno-exceptions -fno-rtti -fno-stack-protector -no-pie fft.cpp \-I/home/arjun/ezh/gem5/include \-L/home/arjun/ezh/gem5/util/m5/build/x86/out -lm5 -o fft
# cd ../gem5
...
# cd util
# python3 decode_packet_trace.py ~/ezh/fft_workload/m5out_fft/trace.ptrc.gz ~/ezh/fft/fft_log_ascii_trace
# cd ../../ # repo root
...
for i in {2..16..2}
do
    echo "Begin bankshifting for shift $i"

    if [ "$i" -ne 0 ]; then
        python3 ~/ezh/scripts/rewrite_dramsim3_bankshift.py --indir ~/ezh/fft_workload/chunk_traces/fft_trace_chunks_bitshift_0/ --outdir ~/ezh/fft_workload/chunk_traces/fft_trace_chunks_bitshift_$i --bank-count 16 --bank-lsb 0 --bank-bit-shift $i --iter-set 10 --clobber
    fi
    ...
    do
        ...
        echo "Beginning iteration $j for shift $i"

        mkdir -p ~/ezh/fft_workload/dramsim_results/dramsim_results_bitshift_$i/dramsim_results_$j
        j_fmt=$(printf "%06d" $j)
        f=$(ls ~/ezh/fft_workload/chunk_traces/fft_trace_chunks_bitshift_$i/iter_$j_fmt_*)
...
# python3 ~/ezh/scripts/rewrite_dramsim3_bankshift.py --indir ~/ezh/fft_workload/chunk_traces/fft_trace_chunks_bitshift_0/ --outdir ~/ezh/fft_workload/chunk_traces/fft_trace_chunks_bitshift_2 --bank-count 16 --bank-lsb 0 --bank-bit-shift 2 --iter-set 10
