...
WORKLOAD_DIR=$REPO_ROOT/workloads/fft_workload
...
WORKLOAD_SRC=$WORKLOAD_DIR/$WORKLOAD_NAME.c

M5OUT_DIR=$WORKLOAD_DIR/m5out_${WORKLOAD_NAME}_baseline_riscv
RAW_TRACE=$WORKLOAD_DIR/${WORKLOAD_NAME}_log_raw_baseline_trace_riscv
DRAMSIM_TRACE=$WORKLOAD_DIR/${WORKLOAD_NAME}_dramsim3_baseline_riscv.trace

DRAMSIM_RESULTS_DIR=$WORKLOAD_DIR/dramsim_results_baseline_riscv
...
riscv64-linux-gnu-gcc -O0 \-nostdlib -static -ffreestanding -fno-exceptions \
...
$GEM5_DIR/build/RISCV/gem5.opt \
    ...
    --num-cpus=1 \
    --mem-size=2147483648 \
    --cmd=$WORKLOAD_BIN > $WORKLOAD_DIR/${WORKLOAD_NAME}_log_baseline.txt
